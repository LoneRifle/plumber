...
async function generateAppsJson() {
  try {
    const apps: IApp[] = sortApps(await App.findAll())
    ...
    // Validate the written JSON can be parsed back as IApp[]
    const writtenContent = fs.readFileSync(OUTPUT_PATH, 'utf-8')
    const parsedApps: IApp[] = JSON.parse(writtenContent)
    console.log(`✓ Validated ${parsedApps.length} apps in output file`)
